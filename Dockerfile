...
RUN cargo zigbuild -r \
    --target x86_64-unknown-linux-musl --target aarch64-unknown-linux-musl && \
  mkdir /app/linux && \
  cp target/aarch64-unknown-linux-musl/release/server /app/linux/arm64 && \
  cp target/x86_64-unknown-linux-musl/release/server /app/linux/amd64
...
WORKDIR /app
ARG TARGETPLATFORM
COPY --from=builder /app/${TARGETPLATFORM} /app/server
CMD ["/app/server"]
